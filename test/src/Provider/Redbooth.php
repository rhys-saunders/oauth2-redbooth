<?php
namespace RhysSaunders\OAuth2\Client\Test\Provider;

use RhysSaunders\OAuth2\Client\Provider\Redbooth;
use Mockery as m;
use ReflectionClass;

class RedboothTest extends \PHPUnit_Framework_TestCase
{
    protected $provider;

    protected static function getMethod($name)
    {
        $class = new ReflectionClass('RhysSaunders\OAuth2\Client\Provider\Redbooth');
    ...
    {
        $authUser = json_decode('{"ok": true,"url": "https:\/\/myteam.Redbooth.com\/","team": "My Team","user": "cal","team_id": "T12345","user_id": "U12345"}',true);
        ...
        $provider = m::mock('RhysSaunders\OAuth2\Client\Provider\Redbooth');
        $provider->shouldReceive('getAuthorizedUser')->andReturn($authUser);
        $provider->shouldReceive('getResourceOwnerDetailsUrl')->once()->andReturn('https://Redbooth.com/api/users.info?token=mock_access_token&user=U12345');
    ...
    /**
     * @expectedException \RhysSaunders\OAuth2\Client\Provider\Exception\RedboothProviderException
     */
